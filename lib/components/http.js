...
let Http = function (app, opts) {
    ...
    this.httpServer.events.on('log', formatLogEvent);

    this.httpServer.port = opts.port || DEFAULT_PORT;
    this.httpServer.address = opts.host || DEFAULT_HOST;
...
Http.prototype.start = function (cb) {
    ...
    self.httpServer.events.on('start', (route) => {
        cb();
        console.log('Server started');
    });

    self.httpServer.register(this.plugins, (err) => {
        if(err) {
...
Http.prototype.stop = function (force, cb) {
    ...
    this.httpServer.events.on('stop', (route) => {
        cb();
